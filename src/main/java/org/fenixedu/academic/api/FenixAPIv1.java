...
import org.fenixedu.academic.api.beans.*;
import org.fenixedu.academic.api.beans.publico.*;
...
public abstract class FenixAPIv1 {
    ...
    private FenixPerson.FenixPhoto getPhoto(final Person person) {
        FenixPerson.FenixPhoto photo = null;
        try {
            ...
            if (person.isPhotoAvailableToCurrentUser()) {
                ...
                photo = new FenixPerson.FenixPhoto(type, data);
            }
        } catch (Exception npe) {
        }
        return photo;
    ...
    public FenixPerson person() {
        ...
        final FenixPerson.FenixPhoto photo = getPhoto(person);
    ...
    public FenixPersonCourses personCourses(@QueryParam("academicTerm") String academicTerm) {
        ...
        List<FenixPersonCourses.FenixEnrolment> enrolments = new ArrayList<>();
    ...
    public void fillEnrolments(final Person person, List<FenixPersonCourses.FenixEnrolment> enrolments, ExecutionSemester executionSemester) {
        ...
        if(foundStudent != null) {
            for (Registration registration : foundStudent.getAllRegistrations()) {
                for (Enrolment enrolment : registration.getEnrolments(executionSemester)) {
                    ...
                    enrolments.add(new FenixPersonCourses.FenixEnrolment(executionCourse, grade, ects));
                }
            }
        }
    }
    ...
    private FenixCalendar getFenixCalendar(String academicTerm, List<EventBean> eventBeans, EventType eventType) {
        ...
        List<FenixCalendar.FenixCalendarEvent> events = new ArrayList<>();
        for (EventBean eventBean : eventBeans) {
            ...
            FenixCalendar.FenixCalendarEvent event = null;

            switch (eventType) {
            case CLASS:
                final Shift classShift = ((ClassEventBean) eventBean).getClassShift();
                event = new FenixCalendar.FenixClassEvent(eventPeriod, rooms, title, new FenixCourse(classShift.getExecutionCourse()));
                break;
            ...
            case EVALUATION:
                ...
                event = new FenixCalendar.FenixEvaluationEvent(eventPeriod, assignedRoom, rooms, title, fenixCourses);
                break;
            }
    ...
    private List<FenixCurriculum> getStudentStatistics(Set<Registration> registrationsList) {
        ...
        final List<FenixCurriculum> curriculums = new ArrayList<>();
    ...
    public FenixPayment personPayments() {
        ...
        List<FenixPayment.PaymentEvent> payed = new ArrayList<>();
        ...
        for (Entry entry : person.getPayments()) {
            ...
            payed.add(new FenixPayment.PaymentEvent(id, amount, name, description, shortDescription, date));
        }

        List<Event> notPayedEvents = calculateNotPayedEvents(person);
        List<FenixPayment.PendingEvent> notPayed = new ArrayList<>();
        ...
        for (Event event: notPayedEvents) {
            ...
            for (Debt debt : calculator.getDebtsOrderedByDueDate()) {
                if (BigDecimalUtil.isPositive(debt.getTotalOpenAmount())){
                    ...
                    notPayed.add(new FenixPayment.PendingEvent(id, description, new FenixPeriod(debtCreation, debtDueDate), entity, reference, amount));
                }
            }
        }
